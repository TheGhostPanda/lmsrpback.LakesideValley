...
//_retrievePrice = [_className,__GETC__(life_garage_prices)] call TON_fnc_index; //Edited, why would you pay for retrieve ?
_sellPrice = [_className,__GETC__(life_garage_sell)] call TON_fnc_index;
//_retrievePrice = if(_retrievePrice == -1) then {1000} else {(__GETC__(life_garage_prices) select _retrievePrice) select 1;}; //Edited, why would you pay for retrieve ?
